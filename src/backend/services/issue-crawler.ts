...
export function parseGeneratedIssues(rawText: string): GeneratedIssue[] {
...
export function deduplicateByTitle(issues: GeneratedIssue[]): GeneratedIssue[] {
...
export function buildSources(sourceIndices: number[], articles: NewsArticle[]): IssueSource[] {
...
export function buildPromptWithArticles(
...
export function buildFallbackPrompt(regionName: string, districtSuffix: string): string {
...
export async function crawlIssuesForDistrict(
  ...
  if (force) {
    ...
    const { count: todayCount, error: todayCountError } = await admin
      .from('issues')
  ...
  const articles = await fetchDistrictNews(districtName || regionName, regionName);
  ...
  for (const issue of generatedIssues) {
    ...
    const sources = hasRealNews ? buildSources(issue.source_indices, articles) : [];
